...
def build_page_data(content_paths, secret_access=False):
    all_pages = {}
    for content_path in content_paths:
        for path, dirs, files in os.walk(content_path, topdown=True):
            ...
            for page_file in files:
                ...
                if ext == ".yml":
                    ...
                    page_data = yaml.safe_load(
                        open(os.path.join(content_path, path, page_file), "r")
                    )
                    ...
                    if not secret_page or secret_access:
                        ...
                        markdown_path = os.path.join(
                            content_path, path, "{}.md".format(page_key)
                        )
